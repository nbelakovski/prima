...
def test_linear_constraint_object():
    ...
    if hasattr(res, 'fun'):
        ...
        assert np.isclose(res.f, 0.5, atol=1e-6, rtol=1e-6)
    else:
...
def test_nonlinear_constraint_object():
    ...
    if hasattr(res, 'fun'):
        ...
        assert np.isclose(res.f, 1, atol=1e-6, rtol=1e-6)
    else:
        raise Exception("Result has no attribute for function value")
...
def test_unsupported_type_raises_exception():
    ...
    assert e_info.match("Constraint type <class 'dict'> not recognized")
