...
def test_callback_terminate(minimize_with_debugging, backend):
    def callback(x, *args):
        return True
    result = minimize_with_debugging(obj, obj.x0, method='cobyla', callback=callback, options={'backend': backend})
...
def test_minimize_constraint_violation(backend):
